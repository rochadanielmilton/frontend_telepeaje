<template>
  <q-form class="row q-col-gutter-md" @submit="$emit('guardar');">
    ...
    <q-file filled class="col-xs-12 col-md-6" label="Seleccione imagen frontal del vehiculo" v-model="valoresModel.imagen_fronal_vehiculo" counter
      :counter-label="counterLabelFn" max-files="1" multiple accept=".jpg" @update:model-value="previewImage1">
    ...
    <q-file filled class="col-xs-12 col-md-6" label="Seleccione una imagen" v-model="valoresModel.imagen_lateral_vehiculo" counter
      :counter-label="counterLabelFn" max-files="1" multiple accept=".jpg" @update:model-value="previewImage2">
      <template v-slot:prepend>
        <q-icon name="cloud_upload" @click.stop.prevent />
      </template>
      <template v-slot:append>
...
export default {
  ...
  setup (props) {
    ...
    onMounted(async () => {
      console.log('safsa', valoresModel.value)
      imageUrl1.value = `${IMG_URL}${valoresModel.value.imagen_fronal_vehiculo}`
      imageUrl2.value = `${IMG_URL}${valoresModel.value.imagen_lateral_vehiculo}`

      const file = new File([imageUrl1.value], 'imagename.png')
      const file2 = new File([imageUrl2.value], 'imagename.png')
