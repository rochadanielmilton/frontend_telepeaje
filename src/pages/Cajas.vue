  ...
  setup () {
    ...
    const exportar = async (id) => {
      ...
      doc.autoTable({
        ...
        foot: [['RECAUDACION TOTAL', '', '', parseFloat(rows.total_recaudado).toFixed(2)]],
        startY: 2.5,
        ...
        // Header
          ...
          doc.text('FECHA/HORA: ', data.settings.margin.left + 2, 2)
          doc.text(rows.fecha.split('T')[0] + ' / ' + (rows.fecha.split('T')[1]).split('.')[0], data.settings.margin.left + 2.9, 2)
          doc.text('FECHA OPERACION: ', data.settings.margin.left + 4.9, 2)
          doc.text(rows.fecha_operacion, data.settings.margin.left + 6.2, 2)
          doc.text('REGIONAL: ', data.settings.margin.left + 0.1, 2.3)
          doc.text(rows.regional, data.settings.margin.left + 0.8, 2.3)
          doc.text('RETEN: ', data.settings.margin.left + 2, 2.3)
          doc.text(rows.reten, data.settings.margin.left + 2.5, 2.3)
          doc.text('CARRIL: ', data.settings.margin.left + 3.7, 2.3)
          doc.text(rows.reten, data.settings.margin.left + 4.2, 2.3)
          doc.text('Turno: ', data.settings.margin.left + 5.4, 2.3)
          doc.text(rows.turno, data.settings.margin.left + 5.9, 2.3)
    ...
    const guardar = async (update, close) => {
      console.log('guardar')
      caja.value.baja = 0
      if (caja.value.total_apertura == null) {
        console.log('----------me reconoce la validacion de apertura')
        caja.value.total_apertura = 0
      }
      if (caja.value.id) {
